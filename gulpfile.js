...
gulp.task('copy-bootstrap-js', function(done) {
  return gulp.src(['php/**/revws_bootstrap.js']).
...
gulp.task('copy-build', function(done) {
  ...
  return gulp.src(
      ['./build/front-' + ver + '.js', './build/back-' + ver + '.js']).
      pipe(gulp.dest('./build/staging/revws/views/js')).
      on('end', done);
});
...
gulp.task('create-index', function(done) {
  ...
  var tasks = folders.map(
      folder => gulp.src('php/index.php').pipe(gulp.dest(folder)));
  return merge(tasks);
});
...
gulp.task('extract-front-translations', function(done) {
  ...
  gulp.src('js').
      ...
      pipe(gulp.dest('./build')).
      on('end', () => {
        del.sync(['./build/transl.js', './build/transl.js.LICENSE.txt'],
            {force: true});
        done();
      });
});
...
gulp.task('create-translations', function(done) {
  const frontTranslations = readTranslations(
      './build/front-translation-keys.json');
  const backTranslations = readTranslations(
      './build/back-translation-keys.json');
  gulp.src('php/app-translation.php').
...
gulp.task('translate',
    gulp.series('extract-front-translations', 'extract-back-translations',
        'create-translations', 'merge-translations-keys'));

gulp.task('stage',
    gulp.series('copy-text-files', 'copy-binary-files', 'copy-bootstrap-js',
        'copy-build', 'create-index', 'create-config-xml', 'translate'));
...
gulp.task('check-license-php', function() {
  return gulp.src(['php/**/*.php', '!php/translations/**']).pipe(license({
...
gulp.task('check-license-tpl', function(done) {
  return gulp.src(['php/**/*.tpl']).pipe(license({
...
gulp.task('check-license',
    gulp.series('check-license-php', 'check-license-tpl'));

gulp.task('build', gulp.series('clean', 'build-javascript', 'copy-javascript'));

gulp.task('release',
    gulp.series('clean', 'check-license', 'build-javascript', 'copy-javascript',
        'stage', 'create-zip'));

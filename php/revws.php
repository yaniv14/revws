...
require_once __DIR__.'/classes/RevwsMail.php';
...
class Revws extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->confirmUninstall       = $this->l(
            'Are you sure you want to uninstall the module? All its data will be lost!'
        );
        $this->ps_versions_compliancy = array('min' => '1.6', 'max' => '1.6.999');
        $this->controllers            = array('MyReviews', 'AllReviews', 'mail', 'dailycheck');
    }
    ...
    public function executeSqlScript($script, $check = true)
    {
        ...
        $sql = str_replace(['PREFIX_', 'ENGINE_TYPE', 'CHARSET_TYPE', 'COLLATE_TYPE'],
            [_DB_PREFIX_, _MYSQL_ENGINE_, 'utf8mb4', 'utf8mb4_unicode_ci'],
            $sql);
        $sql = preg_split("/;\s*[\r\n]+/", $sql);
    ...
    public function hookDisplayRevwsReview($params)
    {
        if (isset($params['review'])) {
            ...
            if ($review) {
                ...
                $this->context->smarty->assign(
                    'linkToProduct',
                    $this->context->link->getProductLink($review->id_entity)
                );
    ...
    public function hookActionOrderStatusUpdate($params)
    {
        ...
        foreach ($products as $product) {
            ...
            try {
                ...
                $o->id_customer = $id_customer;
                $o->token       = Tools::encrypt('revws'.(string)$params['id_order'].(string)$order->id_customer.(string)$product['product_id']);
                $o->sent        = 0;
                $o->reviewed    = 0;
                $now            = new DateTime;
                $now->modify('+7 day');
                $o->send_date = $now->format('Y-m-d H:i:s');
                $o->save();
            } catch (Exception $e) {
    ...
    public function hookActionGetConseqsTriggers()
    {
        ...
        return [
            'reviewCreated'  => new \Revws\ConseqsTrigger(
                $this->l('Revws: Review created'),
                $this->l('Executed when review is created'),
                'actionRevwsReviewCreated'
            ),
            'reviewUpdated'  => new \Revws\ConseqsTrigger(
                $this->l('Revws: Review updated'),
                $this->l('Executed when review is updated'),
                'actionRevwsReviewUpdated'
            ),
            'reviewDeleted'  => new \Revws\ConseqsTrigger(
                $this->l('Revws: Review deleted'),
                $this->l('Executed when review is deleted'),
                'actionRevwsReviewDeleted'
            ),
            'reviewApproved' => new \Revws\ConseqsTrigger(
                $this->l('Revws: Review approved'),
                $this->l('Executed when review is approved'),
                'actionRevwsReviewApproved'
            ),
        ];
    }
